# Policy Sim: how it works

*This is the full technical write-up. It was the README until it was trimmed to a title in commit 3ab512f; the content is kept here so the methodology, the sourcing and the known limitations stay on the record.*
...
### Every proposal sits on top of existing law

A policy is never written on a blank page. Raising the income tax changes a rate
table that already exists; a paid-leave change edits sections that are already
there. So the service names them, from
[DCCouncil/law-xml-codified](https://github.com/DCCouncil/law-xml-codified) —
the Council's own codified XML, the thing code.dccouncil.gov is built from.
**24,113 sections across 55 titles**, indexed by citation and heading.

- A tax answer opens with *"This amends the rate table in D.C. Code
  47-1806.03"* and cites the section.
- An open question surfaces the sections whose headings actually match it —
  *"should DC build more affordable housing"* returns three "Affordable
  housing" sections — offered as a drafting starting point, not a legal opinion.
- Questions with no statutory hook get **nothing**. Potholes, diabetes and a
  child transfer return no section at all, because a wrong statute cited
  confidently is worse than no statute.

Getting that last part right took a fix worth recording. Ranking matches by
*count* put "Buildings exceeding 60 feet in height" above "Affordable housing"
for a housing question, because *build* and *more* are two words and
*affordable* is one. Ranking by how much distinctive language matched gets the
pair the right way round.

**And it buys a real check.** `model/dc_tax.py` was transcribed from the Office
of Tax and Revenue's website. The Council publishes the same schedule as
enacted law, from a different system. `test_the_tax_schedule_matches_the_codified_statute`
asserts they agree bracket for bracket — a transcription slip would now have to
happen identically in two independent places to survive.

The corpus is cloned, not vendored (`bash dc_api/setup_dc_code.sh`); the index
is what's committed.
...
```bash
python -m pytest app/tests dc_api -q   # 165 tests
